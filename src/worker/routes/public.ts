...
publicRoutes.get("/cek-pembayaran/:nomor_objek_pajak", async (c) => {
  try {
    ...
    const surat = await c.env.DB.prepare(
      "SELECT s.nomor_objek_pajak, s.nama_wajib_pajak, s.alamat_objek_pajak, st.tahun_pajak, s.jumlah_pajak_terhutang, st.status_pembayaran, d.nama_dusun FROM surat_pbb s JOIN surat_pbb_tahun st ON s.id = st.id_surat_pbb JOIN dusun d ON s.id_dusun = d.id WHERE s.nomor_objek_pajak = ? AND st.tahun_pajak = ?"
    )
